...
const coverageHoursData: CategoryBarChartProps['data'] = [
  { label: 'Break Time', value: 60 * 60 * 3 * 1000 + 60 * 51 * 1000 }, // 3 hours 51 minutes
  { label: 'Active Care', value: 60 * 60 * 6 * 1000 + 60 * 27 * 1000 }, // 6 hours 27 minutes
  { label: 'Overtime', value: 60 * 60 * 2 * 1000 + 60 * 23 * 1000 }, // 2 hours 23 minutes
];

export default function WidgetDailyWorkHours({
  ...rest
}: React.HTMLAttributes<HTMLDivElement>) {
  const TOTAL_TIME = coverageHoursData.reduce((acc, curr) => curr.value + acc, 0);
    ...
    <WidgetBox.Root {...rest}>
      ...
      <div className='space-y-4 pt-4'>
        <div className='text-paragraph-md text-text-sub-600'>
          ...
          minutes of coverage today 📊
        </div>
        <CategoryBarChart data={coverageHoursData} />
      </div>
    </WidgetBox.Root>
  );
}
